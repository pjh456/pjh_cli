...
int main(int argc, char **argv)
{
    ...
    auto &server_start =
        server.add_command(
            "start",
            "Start server");
    server_start.option<int, fixed_string("port")>(
        "--port",
        'p',
        "Port",
        8080);
    server_start.option<bool, fixed_string("daemon")>(
        "--daemon",
        'd',
        "Run as daemon");
    ...
    if (ctx.has<fixed_string("port")>())
        std::cout
            << ", port: "
            << ctx.get<int, fixed_string("port")>();
    if (ctx.has<fixed_string("daemon")>())
        std::cout
            << ", daemon: "
            << ctx.get<bool, fixed_string("daemon")>();
    if (ctx.has<fixed_string("format")>())
        std::cout
            << ", format: "
            << ctx.get<std::string, fixed_string("format")>();
    std::cout << "\n";
    return 0;
}
